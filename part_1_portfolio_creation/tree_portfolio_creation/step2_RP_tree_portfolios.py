...
# ProcessPool: one panel copy per worker (same pattern as step2_tree_portfolios).
_RP_MP_PANEL: pd.DataFrame | None = None


def _rp_mp_init_worker(panel: pd.DataFrame) -> None:
    global _RP_MP_PANEL
    _RP_MP_PANEL = panel


def _rp_mp_run_single_tree(task: tuple) -> tuple:
    """
    One RP tree. task = (t_idx, file_id, child_seed, feat1, feat2, tree_depth, q_num, y_min, y_max).
    Returns (t_idx, file_id, proj_matrix, ret_table, feat_min_tables, feat_max_tables).
    """
    global _RP_MP_PANEL
    if _RP_MP_PANEL is None:
        raise RuntimeError("RP multiprocessing worker: panel not initialized")
    t_idx, file_id, child_seed, feat1, feat2, tree_depth, q_num, y_min, y_max = task
    ...
    ret_table, feat_min_tables, feat_max_tables = compute_one_rp_tree(
        _RP_MP_PANEL, feat_cols, proj_matrix, tree_depth, q_num, y_min, y_max
    )
    return t_idx, file_id, proj_matrix, ret_table, feat_min_tables, feat_max_tables
...
def create_rp_tree_portfolio(feat1: str,
    ...
    Parallelism: when ``SEMINAR_TREE_BUILD_WORKERS`` is unset, uses up to
    ``min(cpu_count-1, n_trees, 16)`` processes (same numerics as sequential).
    Set ``SEMINAR_TREE_BUILD_WORKERS=1`` for sequential; or an explicit integer.
    """
    feat_cols = ['LME', feat1, feat2]
    n_feats   = len(feat_cols)
    ...
    saved_projections: dict[str, np.ndarray] = {}
    ...
    if w_raw.isdigit():
        ...
        n_workers = min(max(1, cpu - 1), n_trees, 16)
        if n_workers >= 2:
            print(
                f"  parallel RP tree build, {n_workers} processes "
                f"(set SEMINAR_TREE_BUILD_WORKERS=1 for sequential)"
            )
    if n_workers > 1:
        n_workers = min(n_workers, n_trees)

    def _save_one_rp_tree(
        file_id: str,
        proj_matrix: np.ndarray,
        ret_table,
        feat_min_tables,
    ...
    if n_workers > 1:
        ...
        tasks = [
            (
                t_idx,
                str(t_idx).zfill(id_width),
                int(child_seeds[t_idx]),
                feat1,
                feat2,
                tree_depth,
                q_num,
        ...
        try:
            with ProcessPoolExecutor(
                ...
                initializer=_rp_mp_init_worker,
                initargs=(panel,),
            ) as ex:
                futures = [ex.submit(_rp_mp_run_single_tree, t) for t in tasks]
                results = []
                for k_done, fut in enumerate(as_completed(futures), start=1):
                    results.append(fut.result())
                    print(
                        f"  [RP trees] {k_done}/{n_trees} finished "
                        f"({100.0 * k_done / n_trees:.1f}%)",
                        flush=True,
                    )
        except Exception as e:
            print(f"  Parallel RP build failed ({e}); falling back to sequential.")
            results = []
            for t_idx in range(n_trees):
                ...
                results.append(
                    (t_idx, file_id, proj_matrix, ret_table, feat_min_tables, feat_max_tables)
                )
        results.sort(key=lambda x: x[0])
        if n_workers > 1:
            print(f"  Writing {n_trees} trees to CSV...", flush=True)
        for t_idx, file_id, proj_matrix, ret_table, feat_min_tables, feat_max_tables in results:
            if n_workers <= 1:
                print(f"  Tree {t_idx + 1:02d}/{n_trees}: id={file_id}  (saved)")
            _save_one_rp_tree(file_id, proj_matrix, ret_table, feat_min_tables, feat_max_tables)
    else:
        for t_idx in range(n_trees):
            file_id = str(t_idx).zfill(id_width)
            rng = np.random.default_rng(int(child_seeds[t_idx]))
