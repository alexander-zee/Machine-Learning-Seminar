...
def lasso_cv_helper(ports_train, ports_valid, ports_test, lambda0, lambda2, main_dir, sub_dir, adj_w, n_total, cv_name, train_weights, kmin=5, kmax=50, RunParallel=False, ParallelN=10):
    # Moments: match 0_code/2_AP_Pruning/lasso_valid_par_full.R when weights are uniform;
    # weighted path is an extension for kernel moments (not in baseline R).
    X = ports_train.values
    if train_weights is not None and not np.allclose(train_weights, train_weights[0]):
        mu = np.average(X, axis=0, weights=train_weights)
        sigma = np.cov(X, rowvar=False, aweights=train_weights, ddof=1)
    else:
        mu = np.mean(X, axis=0)
...
def _process_lambda0(i, l0_val, lambda0, lambda2, sigma_tilde, mu_tilde, ports_train, ports_valid, ports_test, adj_w, kmin, kmax, main_dir, sub_dir, cv_name):
    ...
    for j, l2_val in enumerate(lambda2):
        ...
        for r in range(n_res):
            ...
            # R: b = b / abs(sum(b))  (lasso_valid_par_full.R) — NOT L1 normalization
            s = np.sum(b)
            den = np.abs(s) if np.abs(s) > 1e-15 else 1e-15
            b = b / den
