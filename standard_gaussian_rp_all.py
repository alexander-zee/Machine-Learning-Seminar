"""
Gaussian kernel grid + full fit for **RP tree** portfolios (same **36** triplets as
``cross_section_triplets.all_triplet_pairs()``: all secondary pairs, **including IdioVol**).

Writes, for each ``LME_feat1_feat2``:

- ``data/results/grid_search/rp_tree/gaussian/LME_*/*`` — validation CSVs from ``AP_Pruning``
  (column depth logic matches RP ``*.node`` names, same as AP).
- ``.../gaussian/LME_*/full_fit/full_fit_detail_k10.csv`` — needed for
  ``export_table51_rp_uniform_vs_gaussian.py`` **Gaussian** SR and FF5 alpha.

**Prerequisites:** ``data/results/rp_tree_portfolios/LME_* / level_all_excess_combined.csv``
and ``data/state_variables.csv``. Uniform RP grid under ``rp_tree/LME_*`` can already exist;
Gaussian outputs go only under ``rp_tree/gaussian/``.
...
From repo root::
    ...
    # Default 4 workers; resume skips rows already ``done`` in progress CSV
    python standard_gaussian_rp_all.py

    # Optional: set workers via environment
    set RP_N_WORKERS=8
    python standard_gaussian_rp_all.py

Resume-safe: progress CSV tracks ``pending`` / ``done`` per triplet.

After Gaussian finishes, run exponential batch + export Table 4::

    python standard_exponential_rp_all.py --workers 8
    python part_3_metrics_collection/export_table51_rp_uniform_vs_gaussian.py --rows no-idiovol --k 10 --latex-out Figures/tables/tab_rp_three_kernels.tex
"""
...
PROGRESS_PATH = GRID_SEARCH_PATH / "gaussian" / "progress_standard_gaussian_rp.csv"
SUMMARY_PATH = GRID_SEARCH_PATH / "gaussian" / "all_cross_sections_summary_standard_gaussian_rp.csv"
...
def run_one(args):
    ...
    try:
        ...
        print(f"  [{row_prefix}] [{subdir}] Starting (RP Gaussian)...", flush=True)
...
if __name__ == "__main__":
    pa = argparse.ArgumentParser(
        description="RP Gaussian kernel: grid + full_fit for 36 triplets (includes IdioVol)."
    )
    pa.add_argument(
        "--workers",
        type=int,
    ...
    print(f"Grid root:     {GRID_SEARCH_PATH} / gaussian / …")
    ...
    if len(pending) == 0:
        ...
        for feat1, feat2 in tqdm(
            ...
            desc="RP Gaussian grid + full_fit",
            unit="triplet",
            file=sys.stderr,
        ):
            row_num = PAIR_TO_ROW.get((feat1, feat2), None)
