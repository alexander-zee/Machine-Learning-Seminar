...
SDF_Alpha_plot_ML = function(portN,SDF_Alpha_Path,OrderPath,Plot_Path){
  graph.width <- 14.7 #16
  graph.height <- 5.7 #9
  text.size <- 16 #20
  ...
  FF_alpha = cbind(alpha[order(sr),c(4, 54, 58, 62, 66, 70, 74)])
  
  plot_name = paste(Plot_Path,"/SDF_Prediction_Methods_tstat_FF5_", p ,".png", sep = '')
  ids = factor(FF_alpha$Id, levels = FF_alpha$Id)
  FF_alpha_gg = rbind(data.frame(Id = ids, Type = "AP-Tree", Alpha = FF_alpha[,2]),
                      data.frame(Id = ids, Type = "V-Tree", Alpha = FF_alpha[,3]),
                      data.frame(Id = ids, Type = "DL-LS", Alpha = FF_alpha[,6]),
                      data.frame(Id = ids, Type = "DL-MV", Alpha = FF_alpha[,7]),
                      data.frame(Id = ids, Type = "RF-LS", Alpha = FF_alpha[,4]),
                      data.frame(Id = ids, Type = "RF-MV", Alpha = FF_alpha[,5]))
